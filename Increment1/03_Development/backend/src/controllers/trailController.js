  ...
  try {
    ...
    const trailResult = await client.query(
      `INSERT INTO trekking_trails
        (approved_by, trail_name, difficulty_level, duration_days, max_altitude, description, region, gpx_file_path)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`,
      [adminId, trail_name, difficulty_level, parseInt(duration_days), max_altitude || null, description, region, gpxFilePath]
    );
...
export const deleteTrail = async (req, res) => {
  try {
    ...
    for (const img of imagesResult.rows) {
      const filePath = toAbsoluteUploadPath(img.image_path);
      if (filePath && fs.existsSync(filePath)) fs.unlinkSync(filePath);
    }

    const gpxPath = trailResult.rows[0].gpx_file_path;
    if (gpxPath) {
      const fullGpxPath = toAbsoluteUploadPath(gpxPath);
      if (fullGpxPath && fs.existsSync(fullGpxPath)) fs.unlinkSync(fullGpxPath);
    }
...
export const getPublicTrailById = async (req, res) => {
  try {
    ...
    const trailResult = await pool.query(
      `SELECT t.trail_id, t.trail_name, t.difficulty_level, t.duration_days,
              t.max_altitude, t.description, t.region, t.gpx_file_path, t.created_at
       FROM trekking_trails t
       WHERE t.trail_id = $1`,
      [id]
    );
