...
const HomestayCard = ({ homestay, index }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-gold/40 hover:shadow-lg transition-all duration-300 group flex flex-col"
    >
      {/* Image */}
...
const StatPill = ({ icon: Icon, label, value, iconClass = "text-gold" }) => (
  <div className="flex items-center gap-3 bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 border border-white/20">
    <Icon className={`h-4 w-4 ${iconClass} flex-shrink-0`} />
    <div>
      <p className="text-[10px] text-white/50 uppercase tracking-widest font-semibold leading-none mb-0.5">{label}</p>
...
const DifficultyBar = ({ level }) => {
  ...
  const labels = ["Easy", "Moderate", "Difficult", "Extreme"];
  return (
    <div>
      <div className="flex gap-1 mb-1.5">
        {[1,2,3,4].map((n) => (
          <div
            key={n}
            className={`h-1.5 flex-1 rounded-full transition-all ${n <= config.fill ? config.bar : "bg-gray-100"}`}
          />
        ))}
      </div>
...
/* ─────────────────────────────────────────────
   TABS
───────────────────────────────────────────── */
const TABS = ["Overview", "Itinerary", "Homestays"];
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 sm:px-10 pb-14">
          <div className="max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              {/* Title */}
              <h1 className="text-4xl sm:text-5xl lg:text-[3.5rem] font-black text-white mb-6 font-heading leading-[1.1] tracking-tight drop-shadow-lg max-w-3xl">
                {trail.trail_name}
              </h1>
                ...
                <div className="lg:col-span-1">
                  <div className="sticky top-20">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden mb-4">
                      <div className="px-6 py-5 bg-gradient-to-br from-stone-50 to-amber-50/30 border-b border-gray-100">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #C8932A22, #E0B04A33)" }}>
                            <Compass className="h-4 w-4 text-gold" />
                          </div>
                          <div>
                            <h3 className="text-sm font-bold text-charcoal">Trek At A Glance</h3>
                            <p className="text-[11px] text-gray-400">Key facts & details</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="px-6 divide-y divide-gray-50">
                        ...
                        <div className="py-4">
                          <div className="flex items-center justify-between mb-3">
                            <span className="flex items-center gap-2.5 text-sm text-gray-400">
                              <TrendingUp className="h-3.5 w-3.5 text-orange-400" /> Difficulty
                            </span>
                            <span className={`text-xs font-bold px-3 py-1 rounded-full border ring-1 ${diff.badge}`}>
                              {trail.difficulty_level}
                            </span>
                          </div>
                        ...
                        {!homestaysLoading && homestays.length > 0 && (
                          <div className="py-3.5">
                            ...
                            >
                              <span className="flex items-center gap-2.5 text-sm text-gray-400">
                                <Home className="h-3.5 w-3.5 text-amber-400" /> Homestays
                              </span>
                      ...
                      {trail.gpx_file_path && (
                        <div className="px-5 py-4 border-t border-gray-50">
                          <a
                            ...
                            download
                            className="w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold rounded-xl bg-gradient-to-r from-gold to-amber-400 text-white hover:from-gold/90 hover:to-amber-400/90 transition-all shadow-sm shadow-amber-200 hover:shadow-md hover:shadow-amber-200 group"
                          >
                            <Download className="h-4 w-4 group-hover:animate-bounce" />
                            Download GPX File
                          </a>
                          <p className="text-[10px] text-center text-gray-300 mt-2.5 font-medium">
                    ...
                    {!homestaysLoading && homestays.length > 0 && (
                      <button
                        onClick={() => setActiveTab("Homestays")}
                        className="w-full bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100 rounded-2xl px-5 py-4 text-left hover:border-gold/40 hover:shadow-md transition-all duration-200 group"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-bold text-charcoal">Where to Stay</p>
                              <p className="text-xs text-gray-400">{homestays.length} homestays on this trail</p>
                            </div>
                          </div>
                ...
                <>
                  ...
                  <div className="flex items-center justify-between mb-5 pb-4 border-b border-gray-100">
                    <p className="text-sm text-gray-400">
                      Showing <span className="font-semibold text-charcoal">{homestays.length}</span> places to stay
                    </p>
                    <div className="flex items-center gap-1.5 text-xs text-gray-400">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {homestays.map((h, i) => (
                      <HomestayCard key={h.homestay_id} homestay={h} index={i} />
                    ))}
                  </div>
                </>
              )}
