...
export const requestGuideBookingRefund = async (req, res) => {
  ...
  try {
    ...
    const bookingResult = await client.query(
       ...
       FROM guide_package_bookings b
       LEFT JOIN LATERAL (
         SELECT session_id, payment_status, transaction_uuid
         FROM guide_package_payment_sessions
         WHERE booking_id = b.booking_id
         ORDER BY created_at DESC
         LIMIT 1
    ...
    if (booking.status === "refunded" || existingRefundStatus === "refunded") {
      await client.query("ROLLBACK");
      return res.status(400).json({ message: "Booking is already refunded" });
    }

    if (booking.status === "refund_requested" || ["requested", "processing"].includes(existingRefundStatus)) {
      await client.query("ROLLBACK");
    ...
    return res.status(500).json({ message: "Server error requesting guide booking refund" });
  } finally {
    client.release();
  }
};
...
export const reviewGuideBookingRefund = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: "Server error reviewing guide refund" });
  } finally {
    client.release();
  }
};
