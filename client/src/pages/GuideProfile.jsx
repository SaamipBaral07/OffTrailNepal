...
// ╠═════════════════════════════════════════════════════════════════════════════════════════════════════╣
// ║ GET   /api/auth/guide/profile     → authRoutes.js  → authController.getGuideProfile                 ║
// ║ GET   /api/guides/verification-status → guideRoutes.js → guideVerificationController.getMyGuideVerificationStatus║
// ║ PATCH /api/auth/profile-photo     → authRoutes.js  → authController.updateProfilePhoto                ║
// ║ PATCH /api/auth/guide/profile     → authRoutes.js  → authController.updateGuideProfile                 ║
// ║ PATCH /api/auth/guide/bank-details → authRoutes.js → authController.updateGuideBankDetails             ║
// ║ (via ChangePasswordCard component)                                                                  ║
// ║ PATCH /api/auth/guide/password     → authRoutes.js → authController.updateGuidePassword                ║
// ╚═════════════════════════════════════════════════════════════════════════════════════════════════════╝
...
import { Loader2, Save, UserRound, Mail, Phone, MapPin, CalendarDays, ArrowRight, Camera, BadgeCheck, Award, Building2, CreditCard, X } from "lucide-react";
...
const GuideProfile = () => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      const res = await api.get("/api/auth/guide/profile");
      setForm({
        full_name: res.data.profile.full_name || "",
        email: res.data.profile.email || "",
        phone: res.data.profile.phone || "",
        license_no: res.data.profile.license_no || "",
        experience_years: String(res.data.profile.experience_years ?? ""),
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const res = await api.patch("/api/auth/guide/profile", payload);
      ...
      showNotice("Guide profile updated successfully");
    } catch (err) {
      showNotice(err.response?.data?.message || "Failed to update guide profile", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSaveBankDetails = async (event) => {
    ...
    try {
      ...
      const res = await api.patch("/api/auth/guide/bank-details", payload);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#f4f2ee] via-[#faf8f4] to-[#f2efe8] font-body">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-32 pb-20">
        <section className="rounded-3xl border border-gold/20 bg-white/95 shadow-[0_10px_30px_rgba(12,35,64,0.08)] p-6 sm:p-8 mb-6">
          <p className="uppercase text-[11px] tracking-[0.24em] text-gold-dark font-semibold mb-2">Guide Space</p>
          <h1 className="text-3xl sm:text-4xl font-heading text-charcoal">My Profile</h1>
          <p className="text-gray-500 mt-2">Keep your credentials and guide details fresh for better trust and bookings.</p>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <aside className="lg:col-span-4 rounded-3xl border border-navy/10 bg-gradient-to-b from-white via-[#fbfaf8] to-gold-pale/30 p-6 shadow-[0_10px_24px_rgba(12,35,64,0.06)] h-fit">
            ...
            <div className="mt-8 space-y-3.5 text-sm rounded-2xl border border-gold/15 bg-gradient-to-br from-white/80 to-gold-pale/40 p-4.5">
              ...
              <div className="flex items-center gap-3 text-gray-700">
                <Award className="h-4 w-4 text-navy flex-shrink-0" />
                <span><span className="font-semibold">{form.experience_years || "0"}</span> years experience</span>
              </div>
              <div className="flex items-center gap-3 text-gray-700">
          ...
          <div className="lg:col-span-8 space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <label className="block text-sm font-semibold text-gray-700">
                    Experience Years
                    <div className="mt-1 relative">
                      <Award className="absolute left-3 top-3.5 h-4 w-4 text-gray-300" />
                      <input
                        type="number"
                        min="0"
            ...
            <ChangePasswordCard
              onNotice={showNotice}
              fallbackEndpoint="/api/auth/guide/password"
              description="Change your guide account password using your current password for verification."
            />
          </div>
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
              <div>
                <h3 className="text-base font-semibold text-gray-900">Guide Bank Details</h3>
                <p className="text-xs text-gray-500 mt-0.5">Provide account details for payout transfers.</p>
              </div>
              <button
                type="button"
