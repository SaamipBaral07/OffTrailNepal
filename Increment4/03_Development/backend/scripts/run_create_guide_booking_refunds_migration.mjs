...
const run = async () => {
  const migrationPath = path.resolve(
    __dirname,
    "../migrations/create_guide_booking_refunds_and_pending_status.sql"
  );
  ...
  try {
    ...
    const refundsTable = await pool.query(
      `SELECT column_name
       ...
       WHERE table_schema = 'public'
         AND table_name = 'guide_booking_refunds'
    ...
    const bookingStatusOk = bookingStatuses.rows.some((row) =>
      String(row.def || "").includes("pending") &&
      String(row.def || "").includes("refund_requested") &&
      String(row.def || "").includes("refunded")
    );

    if (hasRefundsTable && bookingStatusOk) {
      console.log("Migration applied: guide booking refund schema is ready.");
      return;
    }

    console.error("Migration executed, but expected guide refund schema changes were not fully found.", {
