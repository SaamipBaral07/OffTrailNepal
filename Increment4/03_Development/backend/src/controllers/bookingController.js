...
const evaluateRefundPolicy = ({ checkInDate, paidAmount }) => {
  ...
  if (hoursUntilCheckIn < 0) {
    return {
      ...
      reason: "Refund is not available on or after the booking check-in date.",
      hoursUntilCheckIn,
    };
  }
...
const buildHomestayRefundExceptionResponse = (err, action = "request") => {
  const dbCode = String(err?.code || "").trim();

  if (["42703", "42P01"].includes(dbCode)) {
    return {
      status: 500,
      code: "HOMESTAY_REFUND_SCHEMA_ERROR",
      message:
        "Homestay refund service is temporarily unavailable due to a server configuration issue. Please contact support.",
    };
  }
  ...
  if (dbCode === "23503") {
    return {
      status: 409,
      code: "HOMESTAY_REFUND_DATA_INTEGRITY_ERROR",
      message:
        "Refund could not be completed because related booking or payment data is missing. Please contact support.",
    };
  }
  ...
  if (["40001", "40P01"].includes(dbCode)) {
    return {
      status: 409,
      code: "HOMESTAY_REFUND_CONFLICT_RETRY",
      message: "Another refund operation is in progress for this booking. Please retry.",
    };
  }

  return {
    status: 500,
    code: action === "review" ? "HOMESTAY_REFUND_REVIEW_FAILED" : "HOMESTAY_REFUND_REQUEST_FAILED",
    message: action === "review" ? "Server error reviewing refund" : "Server error requesting refund",
  };
};
...
const getValidatedBookingDraft = async ({
  ...
  const configuredPerRoomLimit =
    Number.isFinite(HOMESTAY_MAX_GUESTS_PER_ROOM) && HOMESTAY_MAX_GUESTS_PER_ROOM > 0
      ? Math.floor(HOMESTAY_MAX_GUESTS_PER_ROOM)
      : 2;
  const declaredPerRoomCapacity = Number(homestay.capacity || configuredPerRoomLimit);
  const maxGuestsPerRoom =
    Number.isFinite(declaredPerRoomCapacity) && declaredPerRoomCapacity > 0
      ? Math.max(1, Math.min(configuredPerRoomLimit, Math.floor(declaredPerRoomCapacity)))
      : configuredPerRoomLimit;

  if (roomsBooked > Number(homestay.available_rooms || 0)) {
    return { error: "Not enough rooms available for selected booking", status: 400 };
  }

  if (roomsBooked > guestsCount) {
    return {
      error: "Rooms booked cannot exceed total tourists. At least one tourist is required per room.",
      status: 400,
    };
  }

  if (guestsCount > maxGuestsPerRoom * roomsBooked) {
    return {
      error: `Guest count exceeds the allowed occupancy limit (${maxGuestsPerRoom} tourist(s) per room).`,
      status: 400,
    };
  }
...
export const submitHomestayReview = async (req, res) => {
  ...
  try {
    ...
    if (paymentStatus !== "success") {
      await client.query("ROLLBACK");
      return res.status(409).json({ message: "Only paid and successful stays can be reviewed" });
    }
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT b.*, h.name AS homestay_name, h.location AS homestay_location,
              ...
              (
                b.status = 'confirmed'
                AND COALESCE(LOWER(p.payment_status), LOWER(ps.payment_status), '') = 'success'
                AND CURRENT_DATE > b.check_out_date
                AND hr.review_id IS NULL
              ) AS can_review
       FROM homestay_bookings b
       JOIN homestays h ON b.homestay_id = h.homestay_id
    ...
    const bookings = result.rows.map((booking) => {
      ...
      const policy = evaluateRefundPolicy({
        ...
        paidAmount: booking.paid_amount ?? booking.total_price,
      });

      const isClosed =
        bookingStatus === "cancelled" ||
        bookingStatus === "refunded" ||
        ["processed", "refunded"].includes(refundStatus) ||
        paymentStatus === "refunded";

      const isRefundPending =
        bookingStatus === "refund_requested" ||
        ["requested", "processing", "approved"].includes(refundStatus) ||
    ...
    const result = await pool.query(
      `SELECT b.*, h.name AS homestay_name, h.location AS homestay_location,
              ...
              CASE
                ...
                WHEN b.status = 'refunded'
                  OR LOWER(COALESCE(r.refund_status, '')) IN ('processed', 'refunded')
                  OR LOWER(COALESCE(p.payment_status, ps.payment_status, '')) = 'refunded'
                  THEN 'refunded'
                WHEN b.status = 'refund_requested'
                  OR LOWER(COALESCE(r.refund_status, '')) IN ('requested', 'processing', 'approved')
                  OR LOWER(COALESCE(p.payment_status, ps.payment_status, '')) = 'refund_requested'
                  THEN 'refund_in_process'
                WHEN b.status = 'confirmed' THEN 'confirmed'
                ELSE LOWER(COALESCE(b.status, 'confirmed'))
              END AS host_display_status,
              hr.review_id, hr.rating AS review_rating, hr.comment AS review_comment, hr.created_at AS review_created_at
       FROM homestay_bookings b
       JOIN homestays h ON b.homestay_id = h.homestay_id
...
export const requestBookingRefund = async (req, res) => {
  ...
  try {
    ...
    const bookingResult = await client.query(
      `SELECT b.booking_id, b.status, b.tourist_id, b.check_in_date, b.homestay_id,
    ...
    const booking = bookingResult.rows[0];
    const bookingStatus = String(booking.status || "").trim().toLowerCase();
    const existingRefundStatus = String(booking.refund_status || "").trim().toLowerCase();
    ...
    if (bookingStatus !== "confirmed") {
      await client.query("ROLLBACK");
      return res.status(409).json({
        message: "Only confirmed bookings are eligible for tourist refund requests.",
        refund_eligible: false,
        rule_code: "invalid_booking_status",
      });
    }

    if (bookingStatus === "refunded" || ["processed", "refunded"].includes(existingRefundStatus)) {
      await client.query("ROLLBACK");
      return res.status(400).json({ message: "This booking is already refunded" });
    }

    if (bookingStatus === "refund_requested" || ["requested", "processing", "approved"].includes(existingRefundStatus)) {
      await client.query("ROLLBACK");
    ...
    await reconcileHomestayAvailability({ db: client, homestayId: booking.homestay_id });
    ...
    const errorResponse = buildHomestayRefundExceptionResponse(err, "request");
...
export const reviewBookingRefund = async (req, res) => {
  ...
  try {
    ...
    if (action === "reject") {
      ...
      await reconcileHomestayAvailability({ db: client, homestayId: refund.homestay_id });
       ...
       SET payment_status = 'refunded',
           payment_response = COALESCE(payment_response, '{}'::jsonb) || jsonb_build_object(
             ...
             'refund_reference', COALESCE(
               (
                 SELECT br.gateway_refund_reference
                 FROM booking_refunds br
                 WHERE br.booking_id = $1
                 ORDER BY br.refund_id DESC
                 LIMIT 1
               ),
               ''
             )
    ...
    const errorResponse = buildHomestayRefundExceptionResponse(err, "review");
    return res.status(errorResponse.status).json({
      message: errorResponse.message,
      code: errorResponse.code,
    });
